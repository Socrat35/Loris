/**
 * Helper script for the modified STOP-BANG instrument.
 *
...
$(document).ready(function() {
  ...
  $('#data_entry_form input[type="number"]').trigger('change');
});
...
/**
 *
 * Function to add listeners to number inputs.
 *
 */
function addNumberInputListeners() {
  // Add listeners to the number fields used for BMI calculations
  _addBMIListeners();
}

/**
 *
 * Function to add listeners to the number input fields used for BMI
 * calculations.
 *
 * @private
 */
function _addBMIListeners() {
  // For changes on the fields used for BMI calculations
  $('#Vitals_Weight, #Vitals_Height').on('change', function() {
    // Get weight, height, language and row node from the document
    let weight = $('#Vitals_Weight').val();
    let height = $('#Vitals_Height').val();
    let language = $('#test_language_select').val();
    let matchingRowNode = $('tr[key="B_Body_Mass_Index"].calculated-info');
    // If both BMI parameters are defined
    if (weight !== '' && height !== '') {
      // Initialize bmi variable
      let bmi = '';
      // If both parameters parse to non-null values
      if (parseFloat(weight) > 0 && parseInt(height, 10) > 0) {
        // Calculate BMI
        bmi = Math.round((parseFloat(weight) * 10000) / (parseInt(height, 10) * parseInt(height, 10)));
      }
      // If a BMI could be calculated
      if (bmi !== '') {
        // Show the row
        matchingRowNode.removeClass('hidden');
        // empty the cell and add the BMI value to it
        $('td', matchingRowNode).empty().html(`${language === 'fr' ? 'IMC calculée selon les signes vitaux' : 'BMI Calculated According to Vital Signs'}: ${bmi} kg/m²`);
      }
      // If one or more parameter are undefined
    } else {
      // Hide the row
      matchingRowNode.addClass('hidden');
      // Empty the cell
      $('td', matchingRowNode).empty();
    }
  });
}
